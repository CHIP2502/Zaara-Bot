...
// }
    ...
    (async () => {
        ...
        const pidusageModule = require("pidusage");
        const pidusage = await pidusageModule(process.pid);

        const timeStart = Date.now();

        // === NEW: Advice không dùng srod-v2 ===
        let Data = "";
        try {
            const adv = await axios.get("https://api.adviceslip.com/advice");
            Data = adv.data.slip.advice;
        } catch (e) {
            Data = "Không thể lấy lời khuyên!";
        }

        // dịch sang tiếng Việt
        return request(encodeURI(
            `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=vi&dt=t&q=${Data}`
        ), (err, response, body) => {
